...
namespace Complaint_project1.DAL
{
    public class CmpDAL
    {
        ...
        public DataTable view_complaints(BAL.CmpBAL obj)
        {
            ...
            string qry = "select  c.cmpdate,c.cmp_id, p.productid,p.product,u.user_name, c.cmpmsg, c.cmpstatus from Product p  INNER JOIN CompReg c  ON p.productid = c.productid INNER JOIN UserReg u ON u.user_id = c.user_id";
        ...
        {
            string qry = "select  c.cmpdate, c.cmp_id,p.productid,p.product,u.user_name, c.cmpmsg, c.cmpstatus from Product p  INNER JOIN CompReg c  ON p.productid = c.productid INNER JOIN UserReg u ON u.user_id = c.user_id AND c.user_id ='" + obj.user_id + "'";
